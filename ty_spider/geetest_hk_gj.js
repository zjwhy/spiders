...
function g_wb(D0B){
    // D0B  如果传入的是true那就重现生成一个随机数，否则就用初始化的值，
...
var new_encode = function(){
        ...
        if (U2r * (U2r + 1) % 2 + 5 && null == V8B) {
            ...
            var a8B = new Uint8Array(256);


            for (getRandomValues(a8B), l8B = 0; l8B < a8B["length"]; ++l8B) V8B[w8B++] = 255 & a8B[l8B];
    ...
    return J8B["prototype"]["convert"] = v9B, J8B["prototype"]["revert"] = I9B, J8B["prototype"]["reduce"] = Q9B, 
    J8B["prototype"]["mulTo"] = x9B, J8B["prototype"]["sqrTo"] = i9B, F8B["prototype"]["convert"] = L9B, 
    F8B["prototype"]["revert"] = T9B, F8B["prototype"]["reduce"] = M9B, F8B["prototype"]["mulTo"] = G9B, 
    F8B["prototype"]["sqrTo"] = U9B,
    s8B["prototype"]["modPowInt"] = A9B,
    s8B["prototype"]["copyTo"] = g9B, s8B["prototype"]["fromInt"] = D9B,s8B["ZERO"] = H8B(0), s8B["ONE"] = H8B(1),
    s8B["prototype"]["dlShiftTo"] = t9B, s8B["prototype"]["drShiftTo"] = X9B, s8B["prototype"]["lShiftTo"] = B9B, 
    s8B["prototype"]["rShiftTo"] = N9B, s8B["prototype"]["subTo"] = f9B, s8B["prototype"]["multiplyTo"] = o9B, 
    s8B["prototype"]["squareTo"] = j9B, s8B["prototype"]["divRemTo"] = K9B, s8B["prototype"]["invDigit"] = y9B, 
    s8B["prototype"]["isEven"] = b9B, s8B["prototype"]["exp"] = O9B, s8B["prototype"]["toString"] = l9B, 
    s8B["prototype"]["negate"] = m9B, s8B["prototype"]["abs"] = h8B, s8B["prototype"]["compareTo"] = z8B, 
    s8B["prototype"]["bitLength"] = C8B, s8B["prototype"]["mod"] = c9B,
    s8B["prototype"]["bitLength"] = C8B,s8B["prototype"]["clamp"] = P9B, s8B["prototype"]["fromString"] = E9B,P8B["prototype"]["doPublic"] = e9B, P8B["prototype"]["setPublic"] = w9B, P8B["prototype"]["encrypt"] = V9B, P8B;
...
function get_h7z(wb){
    ...
    return new new_encode().encrypt(wb)
}

function get_r7z(y7z,wb){
   
    // Y7z = {"userresponse":"88888008133144","passtime":1603,"imgload":744,"aa":"@.--.,,.,--,--.--.----,!)!)!)!)!)(!!?sststytyv)s)t*tt))yy)s())())()))())t)yt~ttsvss)tyt(yty)tttsscssssssssts~sssss((((((!!($-*q,1232.5.322*2z2232,26,4.222,11112110210111199FY1-111101111110:D92201102129C$,_bIK$*J$)r","ep":{"v":"6.0.9"},"rp":"bc3422e450df0a32809e81180bc2228e"}
    return p7B['Ha'](n0B["encrypt"](JSON["stringify"](y7z), wb))
}
...
function get_data(gt,challenge,wb,gj_encode,x,passtime,imgload){
    
     var Y7z  = {
        "userresponse": i7B["C"](x, challenge),//challenge  请求滑块图片用到的 x滑块识别出来的距离
        "passtime":passtime,//randomInt(3000,6000)//randomInt(1400,2000) ,//c7B["a"]("endTime", V7z["b"])["getTime"]() - c7B["a"]("startTime", V7z["b"]), //开始和结束时间的差值
        "imgload":imgload, //randomInt(300,800),//c7B["a"]("imgload", V7z["b"]), //imgload 图片加载耗时时间 可以设置在300-800的区间
        "aa": gj_encode, //滑动轨迹加密
        //V7z["d"]["referer_encode"]等于true是这个加密 https://www.tianyancha.com/?jsid=SEM-BAIDU-PZ-SY-2021112-JRGW!!gt!!@.--.,,.,--,--.--.----,!)!)!)!)!)(!!?sststytyv)s)t*tt))yy)s())())()))())t)yt~ttsvss)tyt(yty)tttsscssssssssts~sssss((((((!!($-*q,1232.5.322*2z2232,26,4.222,11112110210111199FY1-111101111110:D92201102129C$,_bIK$*J$)r
        //V7z["d"]["referer_encode"] 等于false是 这个铭文@.--.,,.,--,--.--.----,!)!)!)!)!)(!!?sststytyv)s)t*tt))yy)s())())()))())t)yt~ttsvss)tyt(yty)tttsscssssssssts~sssss((((((!!($-*q,1232.5.322*2z2232,26,4.222,11112110210111199FY1-111101111110:D92201102129C$,_bIK$*J$)r
        "ep": {v: "6.0.9"} //i7B["Vb"](V7z)//{v: "6.0.9"} 固定 版本号
      }

      Y7z['rp'] = I0B(gt + challenge["slice"](0, 32) + Y7z["passtime"])

    var h7z = get_h7z(wb);
    // var y7z = {"userresponse":"88888008133144","passtime":1603,"imgload":744,"aa":"@.--.,,.,--,--.--.----,!)!)!)!)!)(!!?sststytyv)s)t*tt))yy)s())())()))())t)yt~ttsvss)tyt(yty)tttsscssssssssts~sssss((((((!!($-*q,1232.5.322*2z2232,26,4.222,11112110210111199FY1-111101111110:D92201102129C$,_bIK$*J$)r","ep":{"v":"6.0.9"},"rp":"bc3422e450df0a32809e81180bc2228e"}

    var r7z = get_r7z(Y7z,wb)
...
function main_encry(gt,challenge,gj_encode,x,passtime,imgload){
    var wb = g_wb();

    
    var encry_str = get_data(gt,challenge,wb,gj_encode,x,passtime,imgload);
...
//
// var challenge = "af430cbeb4466f99b03cd5b9f6b3c60ed8";
//
// var  gj_encode = "B!!Cvvssssswssssswsvwsvswvwsssswwvwssvssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss!!($-YJY?ZF6$)06SX?KD779H$)a?J76FIU4FJW66$)1NH$)DS$)i4q@Z@R$))$)38Z_$)F$)4OV5:RJ3CpJi$)38ZjK7AKD$)(GIV3S7$)BSVK9G7h6b7Hj3?8Ebg$)J5UG$)-73$)DGFIIbBfH_$)JDAd";
//
// var data = main_encry(gt,challenge,gj_encode,110+5)
...
// console.log(i7B['C'](123,'7b80d0d2e9ab72e32973325869a4a76fcd'))