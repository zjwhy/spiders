...
//字符串还原函数和大数组加载到内存
let dec_func = ast.program.body[1].expression.argument.callee.body.body[2];

let dec_func_name = dec_func.declarations[0].id.name;

let a_list = ast.program.body[1].expression.argument.callee.body.body[0];

let a_hy_dec = ast.program.body[1].expression.argument.callee.body.body[1];

let newAst = parser.parse("");

newAst.program.body.push(a_list);
newAst.program.body.push(a_hy_dec);
newAst.program.body.push(dec_func);

eval(generator(newAst).code)
...
traverse(ast,{
        ...
        VariableDeclarator(path){
            if (path.node.id.name ==dec_func_name){
                let binding = path.scope.getBinding(dec_func_name)
