...
function main_gj(track,c_arr,s_str){
    //var arr =   make_gj(x);
    //var arr =  get_track(x);
    var arr = JSON.parse(track);

    var new_arr = traver_gj(arr);

    // c_arr = JSON.parse({a:c_arr})


// console.log(new_arr.length,arr.length,ii)

    var encode_str_one = encode_gj(new_arr);
    // console.log(encode_str)
    var encode_str = encode_two(encode_str_one,c_arr,s_str);

    return {encode_str:encode_str,passtime:arr[arr.length-1][2] - arr[2][2],"encode_str_one":encode_str_one};
}

// console.log(main_gj(110))

//"J,----.----.---,--(!!Ltttsttsyttvtts~st~y~y)t()())()(()()(t)yyyy~t(tyt(ttytyttystssssssss~sssssssssw(!!($,011204/404-6-01101111201021024.4/2121.1120104/011112024.111310:BVF0112/241-:G$)I$/E"
